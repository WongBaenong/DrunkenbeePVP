...
const soulmaster = {
  classcode: "SO",
  rep: [2, 6, 8, 9, 12, 13, 14, 18, 19, 20, 24, 25],
  motion: [
    "섬열파",
    "난화격",
    "여래",
  ...
  exp: [
    ...
    `쿨타임 : 20s
    슈퍼아머 : 없음
    피격 판정 : 경직(강경직), 상태이상(트포 : 동결)
    `,
    //섭물
    `쿨타임 : 24s
    슈퍼아머 : 피격이상 면역
    피격 판정 : 피격이상(홀딩 종료 시 : 넉백)
    `,
    //흡철
    `쿨타임 : 14s
    슈퍼아머 : 피격이상 면역
    ...
    `슈퍼아머 : 피격이상 면역, 상태이상 면역
    피격 판정 :
    경직(강경직), 피격이상(홀딩 종료 : 에어본)
    `,
  ],
  enter: [8, 14, 19],
  enterExp: [
    `트포에 따라 동결시킬 수도 있으며 시전속도가 제법 빠르기 때문에 주의해야한다. 순보 등으로 인해 에어본됐을 때 마무리로 주로 쓰는 편이긴 하지만 거리를 애매하게 주면 동결을 맞을 수도 있다.`,
    `선딜이 제법 길어서 스페가 있으면 충분히 반응할 수 있지만 스페를 빼놓고 기공장과 풍뢰를 쏘면 피하기 난감할 때도 있다. 선딜이 길다고는해도 단순 무빙으로만 피하려면 반응속도가 제법 필요한 편.`,
    `${transStress("기공사의 대표 진입기")}
    <br>
    피격 시 경직이므로 먼거리에서 기공사가 순보를 쓰면 3번까지 쓸 수 있다는 사실을 항상 염두에 둬야한다. 맞는 순간 `,
  ],
  immuneExp: ``,
  stiff: [0],
  sitffExp: ``,
...
const blade = {
  ...
  ${transImg("BL", 1)} ▶ ${transImg("BL", 14)} ▶ ${transImg("BL", 10)}<br>
  일명 날먹콤보. 어퍼 슬래쉬로 띄우기만 하면 바로 저울을 띄울 수 있다.(최소 체력 25% ~ 30% 삭제) 
  견제용으로 블리츠를 사용하지 않았다면 블리츠까지 사용해 반피 삭제 가능.
  <br><br>
  ...
  <br>
  서프라이즈로 띄우고 어퍼로 다시 띄운 후 헤드헌트를 섞어 저울을 초기화하는 콤보.
  <br>
  3초 이상 피격이상이 유지되면 저울이 떠서 문라 데미지가 덜 들어가기 때문에 서프라이즈 > 어퍼 > 보이드로 바로 연계하는 것보다 중간에 헤헌을 섞어 저울을 늦춘다.
  <br><br>
  ${transImg("BL", 11)} ▶ (${transImg("BL", 17)}) ▶ ${transImg(
    "BL",
    1
  )} ▶ ${transImg("BL", 14)} ▶ ${transImg("BL", 10)}
  <br>
  악셀을 맞춰 상대가 경직인 상태라면 헤헌을 쓰지 않고 바로 어퍼로 띄워 날먹콤보를 사용하면 된다.
  <br><br>
  ...
  <br>
  아츠 발동 시 버블에 따라 공증이 있기 때문에 각성기로 버블을 채운 후 아츠 공증을 받은 상태로 문라를 사용해 체력 대부분을 삭제할 수 있다. 
  상대 기상기 유무에 따라 버스트, 블리츠 등을 섞을 수 있으며 체방따리의 경우 이 콤보로 원콤도 가능하다.
  <br>
  ${transCombo("BL", 1)}
  `,
};
